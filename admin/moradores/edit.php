...
if(!empty($_POST)){
	...
	if(!v::noWhitespace()->length(4, 20)->notEmpty()->validate($username)){
		$userNameError = "Please, type Username again, between 4 and 20 chars, no white space is allowed";
		$valid=false;
	}
...
*/
	if(!v::notEmpty()->length(1, 5)->validate($unidade)){
		$unidadeError = "Please, type the House ID as A-00 format (letter, slash, number))";
		$valid=false;
	}

	if(!v::length(3, 80)->notEmpty()->validate($name)){
		$nameError = "Please type again the Full Name, between 3 and 80 chars long";
		$valid=false;
	}

	if(!v::email()->validate($email)){
			$emailError = "This e-mail is invalid";
		$valid=false;	
	}
...
<?php include '../../partes/topo_admin_moradores.php'; ?>
                ...
                <div class="col-md-10">
                    ...
                    <div class="row">
                        <div class="col-md-12">
				            <div class="well">
				              <h1 id="tables">Updating <?php echo $name ?> [<?php echo $unidade ?>]</h1>
				            </div>
                        </div>
                    </div>
        ...
        <div class="col-md-8 col-sm-offset-2">
            <form role="form" action="edit.php" method="post">
                ...
                <div class="form-group <?php echo !empty($unidadeError) ? 'has-error' : '';?>">
                    <label class="control-label" for="unidade">House ID</label>
                ...
                <div class="well">
                    ...
                    <a class="btn btn-default" href="lista.php">Back</a>
                </div>
            </form>
        </div>
    </div>
